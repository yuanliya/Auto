...
{
    public partial class FormRPArvStatics : Form
    {
        public FormRPArvStatics()
        {
            InitializeComponent();
        }

        private void FormRPArvStatics_Load(object sender, EventArgs e)
        {
            //allArv = new BindingList<ArchiveInfo>(ArchiveBLL.Instance.Find("*", ""));
            allArv = CallerFactory.Instance.GetService<IArvOpService>().GetArvInStorage();//这样不好，应直接在service中增加统计吗
        ...
        private void DrawChart(string condition, string ChartType)
        {
            ...
            if (condition == "GroupNo")
            {
                ...
                List<ArvBoxDto> loc = CallerFactory.Instance.GetService<IArvOpService>().FindArvLocations(null);
                bindingList = loc.GroupBy(q => q.GetType().GetProperty(condition).GetValue(q, null)).Select(q => new DataMap<string> { Key = q.Key.ToString(), Value = q.Count() }).ToList();
            }
            else
            {
                QueryCondition model = new QueryCondition(condition);
